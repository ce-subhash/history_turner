...
# =========================================================================
# 1. POLICE RIOT BARRICADE (Single Lane: 2.2m total width)
# =========================================================================
def build_police_barricade():
    ...
    # Frame width is 2.1m (scale x=1.05)
    ...
    # 3 Riot Police Officers behind barricade (fitting within 2.0m)
...
# =========================================================================
# 2. WOODEN ROADBLOCK (Single Lane: 2.1m total width)
# =========================================================================
def build_wooden_roadblock():
    ...
    # 2 Side A-Frame posts (at x = -0.95 and +0.95)
    for px in [-0.95, 0.95]:
    ...
    # Horizontal planks (width 2.1m -> scale x=1.05)
    add_cube("PlankTop", (0, 0, 1.05), (1.05, 0.04, 0.10), mat_timber, bevel=0.02)
    add_cube("PlankMid", (0, 0, 0.70), (1.05, 0.04, 0.10), mat_timber, bevel=0.02)
    add_cube("PlankBot", (0, 0, 0.35), (1.05, 0.04, 0.08), mat_timber, bevel=0.02)
    ...
    # NO ENTRY sign plaque (width 1.1m)
...
# =========================================================================
# 3. JUMP RAMP (Single Lane: 2.0m total width)
# =========================================================================
def build_jump_ramp():
    clear_scene()
    ...
    # Ramp surface width 1.9m -> scale x=0.95
    add_cube("RampSurface", (0, 0, 0.42), (0.95, 1.25, 0.05), mat_steel, rot=(ramp_angle, 0, 0), bevel=0.02)

    for sx in [-0.98, 0.98]:
        add_cube(f"RampSide_{sx}", (sx, 0, 0.42), (0.04, 1.25, 0.22), mat_trim)

    # 3 Upward Chevron Arrows
...
# =========================================================================
# 4. MOVING BULL CART (Single Lane: 2.0m total width)
# =========================================================================
def build_bull_cart():
    clear_scene()
    ...
    # 1. Spoked Wheels (at x = -0.92 and +0.92, total width 1.95m)
    for wx in [-0.92, 0.92]:
    ...
    # Axle
    add_cylinder("Axle", (0, 0, 0.70), radius=0.06, depth=1.9, mat=mat_iron, rot=(0, math.radians(90), 0))

    # Cart Bed (width 1.6m -> scale x=0.80)
    add_cube("BedPlatform", (0, 0, 0.80), (0.80, 1.2, 0.06), mat_wood, bevel=0.03)
    ...
    # Cargo on Cart
    add_cube("HayBale1", (-0.35, -0.4, 1.05), (0.35, 0.45, 0.25), mat_hay, bevel=0.05)
    add_cube("HayBale2", (-0.30, -0.3, 1.35), (0.30, 0.40, 0.20), mat_hay, rot=(0,0,math.radians(10)), bevel=0.05)

    add_sphere("GrainSack1", (0.35, -0.3, 1.0), 0.24, mat_sack)
    add_sphere("GrainSack2", (0.30, 0.25, 1.0), 0.22, mat_sack)
...
def run():
    ...
    print("ALL OBSTACLE PROPS RESCALED TO SINGLE LANE SUCCESSFULLY!")
